...
int main(int argc, char **argv) {
    ...
    const std::vector<std::string> output_names{"fmap1", "fmap2"};

    const std::vector<std::string> input_names_fuse{"fmap1", "fmap2"};
    ...
    std::shared_ptr<MNN::Express::Module> module(
            MNN::Express::Module::load(input_names, output_names, "../crestereo_without_flow_extract.mnn", &mConfig));
...
//    MNN::Express::Module::Config mConfig1;
    std::shared_ptr<MNN::Express::Module> module_fuse(
            MNN::Express::Module::load(input_names_fuse, output_names_fuse, "../crestereo_without_flow_fuse.mnn", &mConfig));
...
//        std::cout<<"data_to_gpu time : "<<data_to_gpu<<std::endl;
        ...
        auto outputs = module_fuse->onForward({fmap1, fmap2});
